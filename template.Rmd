...
if (Sys.info()["sysname"] == "Darwin") {  # macOS
  if (!file.exists("/Applications/Utilities/XQuartz.app")) {
    message("🔴 XQuartz is not installed. Please install from https://www.xquartz.org/ and restart your Mac.")
  }
}
...
Here we imported the first data set from the U.S. Census Bureau and named them "data set_uninsured_population\_*year"* from 2010-2023, excl 2020. The dataset originally contains 120 observations and 209 variables with in the observations the metadata and in the columns the total population, uninsured population and the margin of errors of the states in the U.S.
...
```{r head uninsured population}

head(dataset_uninsured_population_2010[c(1,105:11),c(2,4,6,8)])
```

**Second Data set: "SAGDP1 State Annual Gross Domestic Product Summary", Statistic: "Real GDP (millions of chained 2017 dollars)"**

Here we imported the second data set from the Bureau of Economic Analysis of the annual GDP of the states from 1997-2024 and named it "table_GDP". The dataset originally contains 60 observations with all the states and regions of the U.S. and 29 variables with the years.
...
```{r datacleaning of the yearly datasets Uninsured Population 2010,2023 margin of error, include=FALSE}
...
```{r data cleaning taking out the states, include=FALSE}
...
```{r data cleaning taking out metadata, include=FALSE}
...
```{r datacleaning storing strings as numeric values, include=FALSE}
...
```{r new variable uninsured share, include=FALSE}
...
```{r merging yearly datasets, include=FALSE}
...
```{r cleaning data from years and states not included in our main dataset of the uninsured population }
#renaming the table
table_GDP = annual_gdp_Table_4_
...
```{r new variable gdp growth, include=FALSE}
...
```{r visualize spatial, echo = FALSE, out.width= "85%"}
...
#Here I am plotting the rest of the US without Alaska
#getting the US map
...
```{r visualization subgroup, echo = FALSE, out.width= "80%"}
...
```{r visualization event, echo = FALSE, out.width= "80%"}
